...
| "查询开放平台 API 错误码/字段说明" | 搜索开放平台接口文档 | `devdoc` | `devapp` | 查 API 用法和错误码，不是管理企业内部应用 |
| "查开放平台应用 DemoApp 的 appKey" | 查询开发者后台企业内部应用 | `devapp` | `devdoc` | 目标是应用实例和凭证/agentId，不是接口文档 |
| "给应用申请 qyapi_robot_sendmsg 权限" | 管理开放平台应用权限 | `devapp permission` | `devdoc` | 权限点申请属于应用配置变更 |
| "查工作台应用 appXYZ 的详情" | 查询钉钉工作台应用 | `workbench app` | `devapp` | `appXYZ/app001` 这类工作台 app id 不是开放平台 `unifiedAppId` |
| "帮我创建一个应用" | 应用类型不明确 | 先追问 | `devapp create` | 缺少开放平台/工作台/AI/MCP 等上下文，不能默认创建企业内部应用 |
| "做一个 AI 应用" | AI 应用/智能体需求不明确 | 先追问或转对应能力 | `devapp create` | 没有开发者后台企业内部应用标识，不能默认走 OpenDev app CRUD |
| "创建 MCP 服务并配置 HSF tool" | MCP 平台工具配置 | OpenDev MCP 平台流程 | `devapp create` | 这是 connector/tool 配置，不是创建开放平台企业内部应用 |
...
### 2. devapp vs devdoc vs workbench — 应用泛词消歧

`应用` 是泛词，不能直接等价为开放平台企业内部应用。

**用 `devapp` 的场景**：
- "开放平台应用/开发者后台应用/企业内部应用" — 管理开发者后台应用实例
...
**用 `workbench app` 的场景**：
- "工作台应用/钉钉工作台上的应用"
- "应用 app001/appXYZ 的详情" 且没有开放平台上下文

**先追问的场景**：
- "创建应用/做个应用/查应用" 但没有开放平台、工作台、文档、AI、MCP 等上下文
- "AI 应用/智能体" 但没有说明是开发者后台企业内部应用还是其他产品能力
- "MCP 服务/connector/HSF tool" 属于平台配置流程，不要转成 `devapp create`

**判断关键**：是否在管理一个开发者后台企业内部应用实例？是 → `devapp`。是否在查 API 文档？是 → `devdoc`。是否在查钉钉工作台入口应用？是 → `workbench app`。仍不明确 → 追问，不调用写命令。
