...
管理钉钉开放平台企业内部应用。用于应用列表查询、应用详情、创建/修改/删除、凭证读取、权限查询/申请/取消，以及事件订阅和版本发布的目标指令设计。
...
`dws app ...` 可以作为兼容别名，但不要把 `opendev/apps` 作为新的正式命令根。
...
dws devapp permission search
dws devapp permission detail
dws devapp permission add
dws devapp permission remove
dws devapp event list
dws devapp event config
...
P0 可用目标是应用 CRUD 和权限；凭证、网页应用、事件、版本发布依赖后端 MCP tool 是否已发布。
...
`--type` 当前只允许 `internal`，用于防止 Agent 误传其它应用类型；它不作为 MCP 字段下发。
...
目标工具：`list_open_dev_app_events`、`configure_open_dev_app_event`。当前属于待补能力。
...
目标工具：`create_open_dev_app_version`、`check_open_dev_app_version_approval`、`publish_open_dev_app_version`、`get_open_dev_app_version_status`。权限申请中的 `requiredApproval=true` 只把权限写入版本变更，后续由版本链路负责审核选人和发布。
...
| 用户说 | 路由 |
| --- | --- |
| 查应用、搜索应用 | `devapp list` |
| 看应用详情、agentId、clientId | `devapp get` |
| 创建应用 | `devapp create` |
| 改名、改描述、换图标 | `devapp update` |
| 删除应用 | `devapp delete` |
| 查权限、搜索某 API 权限 | `devapp permission list --keyword` |
| 这个权限覆盖哪些 API | `devapp permission list --scope` |
| 申请权限 | `devapp permission add` |
| 移除权限 | `devapp permission remove` |
| 读取 appSecret/clientSecret | `devapp credentials get`，后端未发布时直接说明缺口 |
| 事件订阅/发布审核 | 转 `event` 或 `version` 目标链路 |

## 更多设计

详细 helper 映射、P0 RPC 出入参契约、Agent 裁剪规则、上线验收清单和验收用例见仓库文档：
